...
def insert_ap(pkt):
    ...
    if verbose: print ('BSSID: %s' %bssid)
    if bssid in aps:
        return
    p = pkt[Dot11Elt]
    if verbose: print ('Dot11Elt: %s' %p)
    cap = pkt.sprintf("{Dot11Beacon:%Dot11Beacon.cap%}"
                      "{Dot11ProbeResp:%Dot11ProbeResp.cap%}").split('+')
    if verbose: print ('CAP: %s' %cap)
...
aps = {}
sniff(iface='mon1', prn=insert_ap, store=False,
      lfilter=lambda p: (Dot11Beacon in p or Dot11ProbeResp in p))


sniff(iface='mon1', prn=printpacket, store=False,
      lfilter=lambda p: (Dot11Beacon in p or Dot11ProbeResp in p))